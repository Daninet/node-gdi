...
const GDIGraphics = (cb) => {
  ...
  return {
    ...
    text (x, y, text, width, height, options = {}) {
      const parameters = {
        bottom: 0x8,
        calcRect: 0x400,
        center: 0x1,
      ...
      if (typeof width === 'object') {
        ...
        const convertedOptions = width;
        if (!util.isObject(convertedOptions) || Object.keys(convertedOptions).some(k => parameters[k] === undefined)) {
          throw new Error('Invalid convertedOptions parameter!');
        }
        const optionValues = Object.keys(convertedOptions).map(o => parameters[o]).reduce((acc, val) => acc | val, 0);
        data.push([5, x, y, -1, -1, optionValues, text]);
        return this;
      }

      if (width === undefined && height === undefined) {
        data.push([5, x, y, -1, -1, 0, text]);
        return this;
      }
      ...
      if (width && height) {
        ...
        if (!util.isObject(options) || Object.keys(options).some(k => parameters[k] === undefined)) {
          throw new Error('Invalid options parameter!');
        }
        const optionValues = Object.keys(options).map(o => parameters[o]).reduce((acc, val) => acc | val, 0);
        data.push([5, x, y, width, height, optionValues, text]);
        return this;
      } else {
