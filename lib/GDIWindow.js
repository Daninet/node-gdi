  ...
  if (msg.type === 'WM_CLOSE') {
    ...
    try {
      currentWindow.__cbResize({
        ...
        width: event.llParam * 96 / currentDpi,
        height: event.hlParam * 96 / currentDpi
      });
    ...
    try {
      currentWindow.__cbMouseMove({
        x: msg.event.llParam * 96 / currentDpi,
        y: msg.event.hlParam * 96 / currentDpi
      });
    } catch (err) {
      console.error(err);
      process.exit(1);
    ...
    try {
      if (msg.type.startsWith('WM_NC')) {
        currentWindow.__cbMouseDown({
          button,
          x: event.llParam * 96 / currentDpi,
          y: event.hlParam * 96 / currentDpi
        });
      } else {
        currentWindow.__cbMouseDown({
    ...
    currentDpi = msg.event.lwParam;
  } else if (msg.type === 'WM_PAINT') {
    if (currentDpi === null) {
      currentDpi = addon.getDpi();
    }
  }
}
